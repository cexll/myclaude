...
func TestClaudeBuildArgs_ModesAndPermissions(t *testing.T) {
	...
	t.Run("new mode opt-in skip permissions with default workdir", func(t *testing.T) {
		...
		want := []string{"-p", "--dangerously-skip-permissions", "--setting-sources", "user", "--output-format", "stream-json", "--verbose", "-"}
		if !reflect.DeepEqual(got, want) {
			t.Fatalf("got %v, want %v", got, want)
		}
	})
	...
	t.Run("resume mode uses session id and omits workdir", func(t *testing.T) {
		...
		want := []string{"-p", "--dangerously-skip-permissions", "--setting-sources", "user", "-r", "sid-123", "--output-format", "stream-json", "--verbose", "resume-task"}
		if !reflect.DeepEqual(got, want) {
			t.Fatalf("got %v, want %v", got, want)
		}
	})
	...
	t.Run("resume mode without session still returns base flags", func(t *testing.T) {
		...
		want := []string{"-p", "--dangerously-skip-permissions", "--setting-sources", "user", "--output-format", "stream-json", "--verbose", "follow-up"}
		if !reflect.DeepEqual(got, want) {
			t.Fatalf("got %v, want %v", got, want)
		}
	})
